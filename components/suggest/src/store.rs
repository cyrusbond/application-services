    ...
    S: SuggestRemoteSettingsClient,
{
    fn ingest(&self, constraints: SuggestIngestionConstraints) -> Result<()> {
        ...
        for ingest_record_type in ingest_record_types {
            let _ = self.ingest_records_by_type(ingest_record_type, writer, constraints.clone());
        }
    ...
    fn ingest_records(
        ...
        for record in records {
            ...
            if record.deleted {
                ...
                writer.write(|dao| dao.handle_deleted_record(last_ingest_meta_key, record))?;
                continue;
            }
            let Ok(fields) =
